...
function Ball({ power, isCharging, onLaunch, type, resetKey }: { power: number; isCharging: boolean; onLaunch: () => void, type: any, resetKey: number }) {
  ...
  useEffect(() => {
    // Pointermove handles both Mouse and Touch swipes
...
export default function BowlingScene() {
  ...
  return (
    <div className="w-full h-screen bg-black relative select-none overflow-hidden" 
         style={{ touchAction: 'none' }} // Crucial: Stops mobile scroll from breaking physics tracking
         onPointerDown={(e) => { 
      ...
      <Canvas shadows>
        ...
        {/* FIX: Disable camera movement while charging so the swipe controls the ball instead */}
        <OrbitControls 
          enabled={!isCharging} 
          enablePan={false} 
          enableZoom={false} 
      ...
      {/* UI OVERLAY (Kept exactly as stable version) */}
